...
int loadArrPoints(FILE *in_file, points_t &points)
{
    ...
    for (size_t i = 0; i < points.count && rc == PASS; i++) // rc in first
...
int loadPoints(FILE *in_file, points_t &points)
{
    ...
    int rc = loadCountPoints(in_file, points); // работать с полями а не целой струстурой
    if (rc == PASS)
    {
        rc = allocateArrPoint(points); // тоже самое
        if (rc == PASS)
        {
            rc = loadArrPoints(in_file, points); // тожде самое и ренейминг арр удалить
            if (rc != PASS)
                freeArrPoint(points);
        }
    }
...
int getCentrPoints(const points_t &points, point_t &center)
{
    if (points.arr == nullptr)
        return ERR_NULL_PTR;
    ...
    {
        sum_x += points.arr[i].x;
        sum_y += points.arr[i].y;
        sum_z += points.arr[i].z;
    }

    center.x = sum_x / points.count;
    center.y = sum_y / points.count;
    center.z = sum_z / points.count;
